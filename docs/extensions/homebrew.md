...
- **运行态持久化**：`BREW_RUNNING`（`LazyLock<Mutex<Option<BrewRunState>>>`）跨组件生命周期持久化（后台元数据刷新同样占用）。`brew_run` 经 RAII guard（`RunGuard`）占位 + 逐步 `set_step`，Drop 时自动清空 + emit `brew-run-done` 事件。guard 拒绝并发调用（返回错误「已有 Homebrew 操作正在运行」）。前端 `onActivated`（KeepAlive 重进即触发）先查 `brew_run_state`：有残留操作则显示进度 + 监听 `brew-run-done`（常驻到卸载），无则正常加载
