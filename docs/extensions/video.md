...
「模式」选择为列表第一项（通用组，同 image 扩展）；「操作」行有视频或处理中才显示（回车执行主按钮动作：进行中「取消」，否则「开始」，核心未就绪或下载中禁用；孤儿观察态 paths 为空但 busy 置起，该行仅渲染「取消」，保住取消入口）；「输入视频」行承载核心 + 选文件：

- 核心未就绪：副标题提示 +「下载 FFmpeg」（回车同触发下载）
- 下载中：`正在下载核心…` + 进度
- 就绪未选文件：副标题 `核心版本：FFmpeg x.x.x` +「选择」
- 已选单文件：标题文件名、副标题仅元数据（不显示核心版本）
- 已选多文件：标题 `{n} 个视频`、副标题总时长 · 总大小（探测完成度渐进）
- 进行中：副标题 `进度 {i}/{n} · xx%`（单文件 `进度 xx%`）
- 完成后 toast（单文件原文案、多文件汇总），全部成功同时清空选择回到初始态（操作行随之消失），不常驻输出结果行
