...
**跨扩展接收**：finder-ext 等经同页 window CustomEvent `image-pending-input-path` 同步投递路径数组（访达多选区全量带入），`pendingInputPaths` watch（immediate）后按张数分流——单张直达 removeBg（只改本地不落盘默认），多张自动切拼接模式（选区序即拼接序，预览区序号可调）。
