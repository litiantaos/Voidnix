...
框架级配置（全局快捷键）在 `stores/settings.ts`，同样走 `defineConfig`（`config/settings` storePath）。**AI 提供商**（`src/runtime/ai-providers.ts`）：只存 URL/Key/模型（无「使用中」）；列表按提供商分组、**每把 Key 一行**；选用由 agent/translate 等消费者自管；自动写 `ai.env`（`ZHIPU_API_KEY` / `DEEPSEEK_API_KEY` / `ANTHROPIC_*` 等，OpenCode · Grok Build · Claude Code）并幂等装 shell 钩子。详见 [ai-providers.md](./extensions/ai-providers.md)。
