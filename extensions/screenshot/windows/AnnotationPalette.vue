...
const style = computed(() => {
  ...
  const left = leftAligned + paletteW <= props.screenWidth - EDGE_PAD
    ? leftAligned
    : Math.max(EDGE_PAD, Math.min(x + w - paletteW, maxLeft))

  const belowTop = y + h + PALETTE_GAP
  if (belowTop + PALETTE_H <= props.screenHeight - EDGE_PAD) {
    return { top: `${belowTop}px`, left: `${left}px` }
  }

  if (y - PALETTE_H - PALETTE_GAP >= EDGE_PAD) {
    const aboveBottom = props.screenHeight - y + PALETTE_GAP
    return { bottom: `${aboveBottom}px`, left: `${left}px` }
  }

  const insideBottom = props.screenHeight - (y + h) + PALETTE_GAP
  return { bottom: `${Math.max(insideBottom, EDGE_PAD)}px`, left: `${left}px` }
})
</script>
