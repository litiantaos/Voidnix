...
export function useOverlayEvents(options: {
  ...
  function onMouseUp(e?: MouseEvent) {
    ...
    if (options.phase.value === 'select' && options.pendingDrag.value) {
      ...
      const cx = e ? e.clientX : options.crossX.value
      const cy = e ? e.clientY : options.crossY.value
      const sx = options.dragStart.value.x
      const sy = options.dragStart.value.y
      const dx = cx - sx
      const dy = cy - sy
      if (Math.abs(dx) >= DRAG_THRESHOLD || Math.abs(dy) >= DRAG_THRESHOLD) {
        const clampedCx = Math.max(0, Math.min(cx, options.screenW.value))
        const clampedCy = Math.max(0, Math.min(cy, options.screenH.value))
        options.sel.value = {
          x: Math.min(sx, clampedCx),
          y: Math.min(sy, clampedCy),
          w: Math.abs(clampedCx - sx),
          h: Math.abs(clampedCy - sy),
        }
