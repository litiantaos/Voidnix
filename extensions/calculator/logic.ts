...
function tokenize(expr: string): Token[] | null {
  ...
  while (i < expr.length) {
    ...
    if (('0' <= ch && ch <= '9') || ch === '.') {
      let num = ''
      while (i < expr.length && (('0' <= expr[i] && expr[i] <= '9') || expr[i] === '.')) num += expr[i++]
      ...
      const isNegNum = ch === '-' &&
        (tokens.length === 0 || tokens[tokens.length - 1].type === 'lp' || tokens[tokens.length - 1].type === 'op') &&
        i + 1 < expr.length && (('0' <= expr[i + 1] && expr[i + 1] <= '9') || expr[i + 1] === '.')
      if (isNegNum) {
        let num = '-'; i++
        while (i < expr.length && (('0' <= expr[i] && expr[i] <= '9') || expr[i] === '.')) num += expr[i++]
        if (isNaN(parseFloat(num))) return null
