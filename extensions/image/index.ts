...
/** 跨扩展投递的待处理图片路径（finder-ext 等经同页事件写入，View 消费后清空）。 */
...
export default defineExtension({
  ...
  mainView: () => ImageView,
  setup: async () => {
    // 跨扩展同页投递：finder-ext 等经 window CustomEvent 同步写入待处理路径。
    // 同步是硬要求——投递必须先于跳转首帧渲染（列表形状一次到位），异步到达期间
    // 用户 ↓+Enter 会击中「source（选择文件）」行误开系统文件选择器。
    window.addEventListener('image-pending-input-path', (e) => {
      pendingInputPath.value = String((e as CustomEvent<string>).detail || '')
    })
  },
})
