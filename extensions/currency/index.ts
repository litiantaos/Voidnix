...
export default defineExtension({
  ...
  search: {
    dynamic: async (query, ctx): Promise<ProviderResult[]> => {
      ...
      // 全局默认列表（extensionMode=false）空 query 不触发网络请求（避免拖慢主列表）；
      // 扩展内空 query 展示参考汇率
      ...
      if (!trimmed) {
        ...
        return CURRENCIES.filter((c) => c !== base).map((to) => {
          ...
          return {
            ...
            icon: 'i-ri-exchange-cny-line',
            data: { kind: 'extension', value: v.toFixed(4) },
          }
        })
      }
      ...
      const targets = ctx?.extensionMode ? CURRENCIES.filter((c) => c !== fromCurrency) : [primary]
      ...
      for (const toCurrency of targets) {
        ...
        results.push({
          ...
          icon: 'i-ri-exchange-cny-line',
          boost: DYNAMIC_BOOST,
          data: { kind: 'extension', value: converted.toFixed(2) },
        })
      }
