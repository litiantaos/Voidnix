...
fn warm_once() {
    let ok = crate::webkit_tuning::obj_exception::try_block(|| {
        // SAFETY: voidnix_warm_emoji_font 在主线程调用，内部有 @try/@catch 兜底。
        unsafe { voidnix_warm_emoji_font() };
    });
    if ok {
        crate::webkit_tuning::log::component_status(
            "Emoji_Warmer",
            crate::webkit_tuning::log::Status::Enabled,
            None,
        );
    } else {
        crate::webkit_tuning::log::component_status(
            "Emoji_Warmer",
            crate::webkit_tuning::log::Status::Disabled,
            Some("warmer-failed"),
        );
    }
}
