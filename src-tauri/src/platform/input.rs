...
impl Modifier {
    /// 对应 macOS CGEventFlags 位（值取自 `<CoreGraphics/CGEventTypes.h>` 的
    /// `kCGEventFlagMask*` 常量；这里直接硬编码数值避免依赖 core-graphics 版本对齐）。
    const fn flag(self) -> u64 {
        match self {
            Modifier::Cmd => 1 << 20,   // kCGEventFlagMaskCommand = 0x100000
            Modifier::Shift => 1 << 17, // kCGEventFlagMaskShift = 0x020000
            Modifier::Opt => 1 << 19,   // kCGEventFlagMaskAlternate = 0x080000
            Modifier::Ctrl => 1 << 18,  // kCGEventFlagMaskControl = 0x040000
        }
    }
}
...
pub fn post_combo(combo: &str, pid: Option<i32>) {
    ...
    let Some(key_code) = parse_key_code(key_token) else {
        return;
    };
