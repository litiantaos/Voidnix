...
//   - `pub struct *Extension` → register_all() 生命周期注册（消灭 lib.rs 手写清单）
...
function buildExtensionsRs(ids: string[], frameworkCmds: string[], extCmds: string[]): string {
  ...
  const modDecls = ids
    ...
    .join('\n\n')
  // 生命周期 register 列表：与 plugin 同源扫描，消灭 lib.rs 手写双轨
  const registerChain = ids
    .map((id) => {
      const structName = scanExtensionStruct(id)
      return `        .register(${rustName(id)}::${structName})`
    })
    .join('\n')
...
// 扩展命令全局注册（裸名 invoke），插件 invoke_handler 不注册命令。
...
/// 将全部 native 扩展注册进 registry（与 .plugin() 同源扫描，禁止手写清单）。
pub fn register_all(
