...
import { pendingOcrData, pendingOcrImagePath } from './index'
...
watch(
  ...
  (data) => {
    if (!data) return
    // 加载预览图
    if (pendingOcrImagePath.value) {
      imageUrl.value =
        convertFileSrc(pendingOcrImagePath.value) + `?t=${Date.now()}`
    } else {
      imageUrl.value = ''
    }
    runOcr(data)
    pendingOcrData.value = null
    pendingOcrImagePath.value = ''
  },
  { immediate: true },
)
