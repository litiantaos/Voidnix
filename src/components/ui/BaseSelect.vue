...
const calcDropdownStyle = () => {
    ...
    if (spaceBelow >= dropdownHeight) {
        ...
        dropUp.value = true;
        maxHeight =
            Math.floor(spaceAbove / OPTION_HEIGHT) * OPTION_HEIGHT + PADDING;
        top = rect.top - GAP - maxHeight;
    } else {
        dropUp.value = false;
        maxHeight =
            Math.floor(spaceBelow / OPTION_HEIGHT) * OPTION_HEIGHT + PADDING;
        top = rect.bottom + GAP;
    }
...
const onKeyDown = (e: KeyboardEvent) => {
    ...
    if (e.key === "Escape") {
        ...
        const currentOptionIndex = optionIndices.value.indexOf(
            highlightedIndex.value,
        );
