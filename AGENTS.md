...
- **动画启动前双帧等待（paint 对齐）**：所有 adjust 统一经 `scheduleAdjust` 双 rAF 合帧后再执行——若在扩展切换首挂载的重布局完成前启动 animator 动画，NSWindow 逐帧变大而 WebContent 忙于首帧布局、viewport 跟不上动画帧（表现为内容高度明显滞后于窗口高度）；双 rAF 确保一帧完整 layout+paint 后才启动动画（重布局期间 rAF 自然推迟，窗口停留旧高度即 viewport 匹配的稳定态，与 show 路径「先呈现再渐进 animate」同语义）。窗口隐藏时渲染时钟冻结、rAF 停发，adjust 挂起到下次可见才执行（隐藏态本就跳过 set_main_frame，语义一致）
